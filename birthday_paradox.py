...
def our_hash_function(random_key):
   ...
   return int(hashlib.sha3_256(f"{random_key}".encode()).hexdigest(), 16)
...
def how_many_before_collision(length_of_list):
    ...
    while not collision:
        ...
        # hash them, then modulo them, and keep track of these indices
    ...
    print(f"Hashse before collision, {indices_made}, buckets: {length_of_list}, load factor: {indices_made/length_of_list}")
